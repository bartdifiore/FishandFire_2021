...
mod.sample <- glmer(total_insects ~ trout + burn_debris + scale(max_disch_nr_nrst_gage) + scale(preceding_yr_dry_duration_ys) + (1|code) + (1|year), df, family = "poisson")
summary(mod.sample)
...
mod.function <- function(y, data){
    ...
    if(is.integer(data[,y]) == T){ # this just fits a poisson glmer if the repsonse is integer data, but a linear mixed effects model if the response is non-integer
        tidy(glmer(form, data, family = "poisson"), effects = "fixed") %>% mutate(response = y, .before = effect)
      } else{
        tidy(lmer(form, data), effects = "fixed") %>% mutate(response = y, .before = effect)
    }
  }
        # test it 
...
mod.predict <- function(y, data){
    ...
    form = substitute(response ~ trout + burn_debris + scale(max_disch_nr_nrst_gage) + scale(preceding_yr_dry_duration_ys) + (1|code) + (1|year)) # model form
    if(is.integer(data[,y]) == T){ # this just fits a poisson glmer if the repsonse is integer data, but a linear mixed effects model if the response is non-integer
    ggpredict(glmer(form, data, family = "poisson"), terms = ~ burn_debris) %>% 
        as.data.frame() %>% 
        mutate(response = y, .before = x)
        } else{
    ggpredict(lmer(form, data), terms = ~ burn_debris) %>% 
            as.data.frame() %>% 
            mutate(response = y, .before = x)
  }
}
...
ms <- model_sum %>% 
  ...
  filter(term == "scale(max_disch_nr_nrst_gage)" | term == "scale(preceding_yr_dry_duration_ys)") %>% 
  mutate(term = case_when(
    term == "scale(max_disch_nr_nrst_gage)" ~ "Max discharge", 
...
# Conditional effects plots of burn status, holding continuous predictors at their mean, and assuming trout absent. Plots are conditional mean and 95% confidence intervals at the popualtion-level of the random effect. 
