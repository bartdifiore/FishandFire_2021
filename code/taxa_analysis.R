...
taxa <- c("turbellaria", "ept", "trichoptera", "mollusks", "ephemerop", "megaloptera", "diptera", "och")

t.predictors <- c("code", "month", "year","trout", "avg_daily_disch_nr_nrst_gage", "preceding_yr_dry_duration_ys")

t.for_filter <- c( t.predictors, taxa)
...
df <- df %>% 
  ...
  mutate(across(all_of(taxa), round, 0), 
         across(all_of(taxa), as.integer)) %>%
  rename(discharge = avg_daily_disch_nr_nrst_gage) %>%
  mutate(log.discharge = log(discharge), 
         year = as.factor(year)) %>%
  pivot_longer(turbellaria:och) %>%
...
ggsave("figures/taxa_glmer_contrasts.png", width = 6, height = 4)
